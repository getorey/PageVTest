...
export async function executePlanBuild(args) {
    const { classifiedItems, policy = {} } = args;
    const { archiveOldFiles = false, archiveThresholdDays = 365, archiveDestination = 'Archive', createYearFolders = true, } = policy;
    const moves = [];
    const renames = [];
    const archives = [];
    const conflicts = [];
...
//# sourceMappingURL=plan-build.js.map