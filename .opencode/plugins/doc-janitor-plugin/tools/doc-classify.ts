import { z } from 'zod';
import type { FileInfo } from './doc-scan.js';
...
export type DocClassifyArgs = z.infer<typeof DocClassifyArgsSchema>;
...
const DEFAULT_EXTENSION_MAPPING: Record<string, string> = {
  '.pdf': '문서',
  '.doc': '문서',
  '.docx': '문서',
  '.ppt': '발표자료',
...
export async function executeDocClassify(args: DocClassifyArgs): Promise<ClassifiedItem[]> {
  ...
  for (const file of inventory) {
    ...
    if (!isSensitive) {
      for (const [cat, catPatterns] of Object.entries(patterns)) {
        const patternsArray = catPatterns as string[];
    ...
    // 확장자 기반 분류 (패턴 미일치 시)
    const extMapping = extensionMapping as Record<string, string>;
