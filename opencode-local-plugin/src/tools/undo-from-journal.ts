...
export type UndoFromJournalArgs = z.infer<typeof UndoFromJournalArgsSchema>;
...
// 저널 파일에서 특정 ID 찾기
async function findJournalEntry(journalId: string): Promise<JournalEntry | null> {
...
export async function executeUndoFromJournal(args: UndoFromJournalArgs): Promise<UndoResult> {
  ...
  const undoneOperations: UndoResult['undoneOperations'] = [];
  let successful = 0;
  let failed = 0;
