 ...
 * 민감 경로 확인
 */
function containsSensitivePath(args: Record<string, any>): { isSensitive: boolean; matchedPattern: string | null } {
 ...
 * 승인 확인
 */
function hasValidApproval(args: Record<string, any>): boolean {
 ...
 * - plan_apply 승인 확인
 */
export async function beforeToolExecute(context: ToolExecuteContext): Promise<{ allowed: boolean; reason?: string }> {
 ...
 * 감사 로그 저장
 */
async function saveAuditLog(tool: string, args: Record<string, any>, result: any): Promise<void> {
 ...
 * - 감사 로그 기록
 */
export async function afterToolExecute(context: ToolExecuteContext & { result: any }): Promise<void> {
